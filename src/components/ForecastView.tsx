...
const CAT_STYLE: Record<ForecastCategory, { label: string; chip: string }> = {
  commit: { label: "Commit", chip: "bg-emerald-500/15 text-emerald-400" },
  "best-case": { label: "Best case", chip: "bg-amber-500/15 text-amber-400" },
  pipeline: { label: "Pipeline", chip: "bg-ash/10 text-ash" },
};
...
function DealRow({ line }: { line: ForecastLine }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-t border-ash/10 py-2.5 text-sm">
      <div className="min-w-0">
        <div className="truncate font-medium text-bone">
          {line.prospectName}
        </div>
        <div className="truncate text-xs text-ash/70">
        ...
        <div>
          ...
          <ul className="space-y-2">
            {forecast.topInflators.map((l) => (
              <li
                key={l.dealId}
                className="flex items-center justify-between rounded-lg border border-rose-500/30 bg-rose-500/10 p-3 text-sm"
              >
                <span className="text-rose-400">
                  {l.prospectName} — {l.repConfidence}% logged vs{" "}
                  {l.evidenceConfidence}% on the evidence
                </span>
                <span className="shrink-0 font-semibold text-rose-400">
      ...
      <div>
        ...
        <div className="space-y-4">
          {forecast.months.map((m) => (
            <div key={m.month} className="rounded-xl border border-ash/15 p-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-bone">
                  {m.monthLabel}
                </span>
                <span className="text-sm">
                  <span className="text-ash">{PHP.format(m.repWeighted)}</span>
                  <span className="text-ash/50"> → </span>
